...
`EncryptedSession` は、あらゆるセッション実装に対して透過的な暗号化を提供し、自動的に古い項目を期限切れにして会話データを保護します。

## 機能

- **透過的な暗号化**: 任意のセッションを Fernet 暗号化でラップします
- **セッションごとの鍵**: 一意のセッションごとに HKDF による鍵導出を使用します
- **自動有効期限**: TTL が切れた古い項目はサイレントにスキップされます
- **ドロップイン置換**: 既存の任意のセッション実装で機能します

## インストール

暗号化セッションには `encrypt` の extra が必要です:
...
!!! warning "高度なセッション機能"
    ...
    - コンテンツベースの検索は暗号化データ上で実行されるため、効果が制限されます
