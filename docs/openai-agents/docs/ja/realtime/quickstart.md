...
!!! warning "ベータ機能"
リアルタイム エージェントはベータ版です。実装を改善する過程で破壊的な変更が発生する場合があります。
...
- `voice`: 音声タイプを選択（`alloy`、`echo`、`fable`、`onyx`、`nova`、`shimmer`）
- `modalities`: 有効化するモダリティを指定（`["text"]` または `["audio"]`）
...
- [examples/realtime](https://github.com/openai/openai-agents-python/tree/main/examples/realtime) フォルダー内の動作するサンプルを確認
- エージェントにツールを追加
- エージェント間のハンドオフを実装
- 安全のためのガードレールを設定
