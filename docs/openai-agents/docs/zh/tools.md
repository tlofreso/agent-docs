...
-   [`WebSearchTool`][agents.tool.WebSearchTool] 让智能体可进行网络检索。
...
`ShellTool` 还支持由 OpenAI 托管的容器执行。在你希望模型在受管容器中运行 shell 命令、而不是在本地运行时中执行时，请使用此模式。
...
-   通过 `ShellTool(environment={"type": "local", "skills": [...]})` 可使用本地 shell 技能。
...
你可以将任意 Python 函数用作工具。Agents SDK 会自动设置工具：

-   工具名称将是 Python 函数名（或你也可提供名称）
-   工具描述将取自函数的 docstring（或你也可提供描述）
-   函数输入的 schema 会根据函数参数自动创建
-   每个输入的描述取自函数 docstring，除非禁用
...
1.  你可以将任意 Python 类型用作函数参数，函数可以是同步或异步。
2.  若存在 docstring，则会用于提取描述和参数描述
3.  函数可选接收 `context`（必须是第一个参数）。你也可以设置覆盖项，例如工具名称、描述、使用哪种 docstring 风格等。
4.  你可以将装饰后的函数传入工具列表。
...
除返回文本输出外，你还可以将一个或多个图像或文件作为工具调用输出返回。为此，你可以返回以下任意类型：
...
-   文本：字符串或可转字符串对象，或 [`ToolOutputText`][agents.tool.ToolOutputText]（或 TypedDict 版本 [`ToolOutputTextDict`][agents.tool.ToolOutputTextDict]）

### 自定义工具调用

有时你不想把 Python 函数当作工具。你也可以直接创建 [`FunctionTool`][agents.tool.FunctionTool]。你需要提供：
...
如前所述，我们会自动解析函数签名来提取工具 schema，并解析 docstring 来提取工具和各参数的描述。说明如下：

1. 签名解析通过 `inspect` 模块完成。我们使用类型注解来理解参数类型，并动态构建 Pydantic 模型表示整体 schema。它支持大多数类型，包括 Python 基本类型、Pydantic 模型、TypedDict 等。
2. 我们使用 `griffe` 解析 docstring。支持的 docstring 格式有 `google`、`sphinx` 和 `numpy`。我们会尝试自动检测 docstring 格式，但这属于尽力而为；你也可以在调用 `function_tool` 时显式设置。你还可以通过将 `use_docstring_info` 设为 `False` 来禁用 docstring 解析。

schema 提取代码位于 [`agents.function_schema`][]。

### 使用 Pydantic Field 约束并描述参数

你可以使用 Pydantic 的 [`Field`](https://docs.pydantic.dev/latest/concepts/fields/) 为工具参数添加约束（例如数字的最小/最大值、字符串长度或模式）和描述。与 Pydantic 一致，两种形式都支持：基于默认值（`arg: int = Field(..., ge=1)`）和 `Annotated`（`arg: Annotated[int, Field(..., ge=1)]`）。生成的 JSON schema 与校验会包含这些约束。
...
达到超时时，默认行为是 `timeout_behavior="error_as_result"`，会发送模型可见的超时消息（例如 `Tool 'slow_lookup' timed out after 2 seconds.`）。
...
通过 `@function_tool` 创建工具调用时，你可以传入 `failure_error_function`。该函数在工具调用崩溃时向 LLM 提供错误响应。

-   默认情况下（即你未传入任何值），会运行 `default_tool_error_function`，告知 LLM 发生了错误。
-   若你传入自定义错误函数，则改为运行它，并将响应发送给 LLM。
-   若你显式传入 `None`，则任何工具调用错误都会被重新抛出由你处理。这可能是模型生成了无效 JSON 导致的 `ModelBehaviorError`，或你的代码崩溃导致的 `UserError` 等。
...
在某些工作流中，你可能希望由一个中心智能体编排一组专门智能体，而不是进行控制权任务转移。你可以通过将智能体建模为工具来实现。
...
`agent.as_tool` 函数是一个便捷方法，可轻松将智能体转换为工具。它支持常见运行时选项，如 `max_turns`、`run_config`、`hooks`、`previous_response_id`、`conversation_id`、`session` 和 `needs_approval`。它还支持通过 `parameters`、`input_builder` 和 `include_input_schema` 实现结构化输入。对于高级编排（例如条件重试、回退行为或串联多个智能体调用），请在你的工具实现中直接使用 `Runner.run`：
...
`Agent.as_tool(..., needs_approval=...)` 与 `function_tool` 使用相同审批流程。若需要审批，运行会暂停，待处理项会出现在 `result.interruptions` 中；随后使用 `result.to_state()`，并在调用 `state.approve(...)` 或 `state.reject(...)` 后恢复。完整暂停/恢复模式见[Human-in-the-loop 指南](human_in_the_loop.md)。

### 自定义输出提取

在某些情况下，你可能希望在将工具智能体输出返回给中心智能体前进行修改。适用场景包括：

-   从子智能体的聊天历史中提取特定信息（例如 JSON 负载）。
-   转换或重格式化智能体最终答案（例如将 Markdown 转换为纯文本或 CSV）。
-   在智能体响应缺失或格式错误时校验输出或提供回退值。
...
向 `as_tool` 传入 `on_stream` 回调，可在嵌套智能体运行期间监听其发出的流式事件，同时在流完成后仍返回最终输出。
...
- 处理器可为同步或异步；每个事件会按到达顺序传递。
- 当工具通过模型工具调用触发时会提供 `tool_call`；直接调用时它可能为 `None`。
- 完整可运行示例见 `examples/agent_patterns/agents_as_tools_streaming.py`。

### 条件式工具启用

你可以使用 `is_enabled` 参数在运行时按条件启用或禁用智能体工具。这使你能够根据上下文、用户偏好或运行时条件，动态筛选 LLM 可用工具。
...
`codex_tool` 封装了 Codex CLI，使智能体可在一次工具调用期间运行工作区范围任务（shell、文件编辑、MCP 工具）。该接口为实验性，未来可能变化。

当你希望主智能体在不离开当前运行的情况下，将有界工作区任务委派给 Codex 时可使用它。默认工具名是 `codex`。若设置自定义名称，则必须为 `codex` 或以 `codex_` 开头。当一个智能体包含多个 Codex 工具时，每个工具必须使用唯一名称。
...
-   执行范围：`sandbox_mode` 和 `working_directory` 定义 Codex 可操作范围。请配套使用；当工作目录不在 Git 仓库内时设置 `skip_git_repo_check=True`。
-   线程默认值：`default_thread_options=ThreadOptions(...)` 配置模型、推理强度、审批策略、附加目录、网络访问和网络检索模式。优先使用 `web_search_mode`，而非旧版 `web_search_enabled`。
...
-   `use_run_context_thread_id=True` 会在共享同一可变上下文对象的跨运行场景中，在运行上下文里存储并复用线程 ID。
-   线程 ID 优先级为：单次调用 `thread_id`，然后运行上下文线程 ID（若启用），再到配置中的 `thread_id` 选项。
-   默认运行上下文键在 `name="codex"` 时为 `codex_thread_id`，在 `name="codex_<suffix>"` 时为 `codex_thread_id_<suffix>`。可通过 `run_context_thread_id_key` 覆盖。
...
-   流式传输：`on_stream` 接收线程/轮次生命周期事件及条目事件（`reasoning`、`command_execution`、`mcp_tool_call`、`file_change`、`web_search`、`todo_list` 和 `error` 条目更新）。
